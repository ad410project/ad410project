<div class="container-fluid">
    <nav class="navbar fixed-top navbar-expand-sm bg-light justify-content-end">
        <a class="navbar-brand" href="?controller=static&action=landing"><img src="../images/Fun_LogoBanner.png" alt="Logo" class="logo-banner"></a>
        <ul class="navbar-nav">
            <li class="nav-item">
                <a class="nav-link" href="?controller=static&action=registration">Sign Up</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?controller=static&action=login">Login</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?controller=static&action=searchEvents">Search Events</a>
            </li>
        </ul>
    </nav>
</div>