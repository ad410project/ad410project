...
CREATE TABLE IF NOT EXISTS `mydb`.`Children` (
  ...
  PRIMARY KEY (`childId`),
  INDEX `fk_Children_Users1_idx` (`userEmail` ASC),
  CONSTRAINT `fk_Children_Users1`
    FOREIGN KEY (`userEmail`)
    REFERENCES `mydb`.`Users` (`userEmail`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `mydb`.`Organizations` (
  ...
  INDEX `fk_Organizations_userId_id` (`userEmail` ASC),
...
CREATE TABLE IF NOT EXISTS `mydb`.`UserAddresses` (
  `userEmail` VARCHAR(45) NOT NULL,
  `addressId` INT(11) NOT NULL,
  PRIMARY KEY (`userEmail`, `addressId`),
  INDEX `fk_Addresses_has_Users_Users1_idx` (`addressId` ASC),
  INDEX `fk_Addresses_has_Users_Addresses_idx` (`userEmail` ASC),
  CONSTRAINT `fk_Addresses_has_Users_Addresses`
    FOREIGN KEY (`addressId`)
    REFERENCES `mydb`.`Addresses` (`addressId`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
...
-- -----------------------------------------------------
-- Table `mydb`.`EventCategories`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`EventCategories` ;

CREATE TABLE IF NOT EXISTS `mydb`.`EventCategories` (
  `categoryId` INT(11) NOT NULL,
  `eventId` INT(11) NOT NULL,
  INDEX `fk_Categories_has_Events_Events1_idx` (`eventId` ASC),
  INDEX `fk_Categories_has_Events_Categories1_idx` (`categoryId` ASC),
  CONSTRAINT `fk_Categories_has_Events_Categories1`
    FOREIGN KEY (`categoryId`)
    REFERENCES `mydb`.`Categories` (`categoryId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Categories_has_Events_Events1`
    FOREIGN KEY (`eventId`)
    REFERENCES `mydb`.`Events` (`eventId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`EventTypes` ;

CREATE TABLE IF NOT EXISTS `mydb`.`EventTypes` (
  `typeId` INT(11) NOT NULL,
  `eventId` INT(11) NOT NULL,
  INDEX `fk_Types_has_Events_Events1_idx` (`eventId` ASC),
  INDEX `fk_Types_has_Events_Types1_idx` (`typeId` ASC),
  CONSTRAINT `fk_Types_has_Events_Types1`
    FOREIGN KEY (`typeId`)
    REFERENCES `mydb`.`Types` (`typeId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Types_has_Events_Events1`
    FOREIGN KEY (`eventId`)
    REFERENCES `mydb`.`Events` (`eventId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
